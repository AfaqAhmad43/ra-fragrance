...
const COLORS = ["#DFB76C", "#F5E1A4", "#997530", "#C4A25C"];
...
const AdminDashboard = () => {
  ...
  // Active View Tab: 'catalog' | 'analytics'
  const [activeTab, setActiveTab] = useState<"catalog" | "analytics">("catalog");

  // Analytics Data
  const [events, setEvents] = useState(() => getStoredEvents());
  const analyticsSummary = getAnalyticsSummary(events);
  ...
  const handleGalleryFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setActionMessage("Gallery photo added!");
      setTimeout(() => setActionMessage(null), 3000);
    } catch (err) {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground pt-24 pb-20 px-6 font-body">
      <div className="max-w-[1280px] mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground tracking-[0.2em] uppercase">
                Catalog Management & Real-Time Order Analytics
              </p>
            </div>
          </div>
        ...
        {/* Database Status Indicator & View Switcher */}
        <div className="mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          
          {/* View Mode Tabs */}
          <div className="flex items-center gap-2 p-1.5 rounded-full glass-card border border-primary/30">
            <button
              onClick={() => setActiveTab("catalog")}
              className={`px-5 py-2 rounded-full text-xs font-body tracking-[0.15em] uppercase transition-all duration-300 flex items-center gap-2 cursor-pointer ${
                activeTab === "catalog"
            ...
            >
              <Layers size={14} />
              <span>Catalog ({perfumes.length})</span>
            </button>
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {isSupabaseConfigured ? (
              ...
              <span className="inline-flex items-center gap-1 text-amber-400 font-semibold bg-amber-950/40 px-2.5 py-0.5 rounded-full border border-amber-500/30">
                <AlertCircle size={12} /> Local Client Storage
              </span>
            )}
          </div>
        </div>
            ...
            <div className="glass-card rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                <button
                  ...
                  className="text-muted-foreground hover:text-foreground text-xs flex items-center gap-1 transition-colors mr-2"
                >
                  <RotateCcw size={13} />
        ...
        {/* TAB 2: ANALYTICS & DEMAND DASHBOARD */}
        {activeTab === "analytics" && (
          <div className="space-y-8 animate-fade-in">
            
            {/* Analytics Metric Cards */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="glass-card rounded-2xl p-6 border border-primary/30 gold-glow">
                ...
                <p className="font-display text-gold-gradient text-4xl font-bold">
                  {analyticsSummary.totalWhatsAppClicks}
                </p>
                <p className="text-[11px] text-muted-foreground mt-2 flex items-center gap-1">
                  <TrendingUp size={12} className="text-emerald-400" />
                  <span className="text-emerald-400 font-semibold">+18.5%</span> WhatsApp order intent growth
                </p>
              </div>
              ...
              <div className="glass-card rounded-2xl p-6 border border-primary/30">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs uppercase tracking-widest text-muted-foreground font-medium">
                    Most Demanded Perfume
                  </span>
                  <Star size={20} className="text-primary" />
                </div>
                <p className="font-display text-foreground text-3xl font-bold truncate">
                  {analyticsSummary.topPerfume}
                </p>
                <p className="text-[11px] text-primary mt-2 font-medium">
                  {analyticsSummary.topPerfumeClicks} WhatsApp Order Clicks
                </p>
              </div>
              ...
              <div className="glass-card rounded-2xl p-6 border border-primary/30">
                ...
                <p className="text-[11px] text-muted-foreground mt-2">
                  Accounts for ~54% of customer interest
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {/* Bar Chart: Top Demanded Fragrances */}
              <div className="glass-card rounded-3xl p-6 border border-primary/20">
                <h3 className="font-display text-gold-gradient text-xl font-semibold mb-2">
                  Top Demanded Perfumes (Order Clicks)
                </h3>
                <p className="text-xs text-muted-foreground mb-6">
                  Ranks fragrances by customer WhatsApp order intent.
                </p>

                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={analyticsSummary.perfumeRankings}>
                      ...
                      />
                      <Bar dataKey="clicks" fill="#DFB76C" radius={[6, 6, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>

              {/* Area Chart: Weekly Order Intent Trend */}
              <div className="glass-card rounded-3xl p-6 border border-primary/20">
                <h3 className="font-display text-gold-gradient text-xl font-semibold mb-2">
                  Weekly Order Intent Trend
                </h3>
                <p className="text-xs text-muted-foreground mb-6">
                  Daily WhatsApp click trajectory over the past week.
                </p>
                    ...
                    <AreaChart data={analyticsSummary.dailyTrend}>
                      <defs>
                        <linearGradient id="goldGradient" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#DFB76C" stopOpacity={0.8} />
                          <stop offset="95%" stopColor="#DFB76C" stopOpacity={0} />
                        </linearGradient>
                      </defs>
